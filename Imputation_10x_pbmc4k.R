# Prepare data
library(Matrix)
counts <- readMM(file = "F:/data/10x_pbmc4k/filtered_gene_bc_matrices/GRCh38/matrix.mtx")
geneName <- read.table(file = "F:/data/10x_pbmc4k/filtered_gene_bc_matrices/GRCh38/genes.tsv", header = FALSE, stringsAsFactors = FALSE)
geneName <- geneName[,1]
cellName <- read.table(file = "F:/data/10x_pbmc4k/filtered_gene_bc_matrices/GRCh38/barcodes.tsv", header = FALSE, stringsAsFactors = FALSE)
cellName <- cellName[,1]
...
saveRDS(counts, file = "F:/data/10x_pbmc4k/10x_pbmc4k.rds")
...
# PLot Accuracy
load("results_10x_pbmc4k_ImputeSingle.Rdata")
pdf("F:/Plots/ImputeSingle/10x_pbmc4k/Accuracy.pdf", width = 14, height = 9)
par(mfrow=c(3,4))
for(Kcluster in c(2, 5, 10)){
  for(percentage in c(0.05, 0.1, 0.2, 0.5)){
    ...
    for(depth in c(2, 5, 10, 20, 50)){
      resultsName <- paste0("`10x_pbmc4k_ImputeSingle_p_", percentage, "_K_", Kcluster, "_d_", depth, "`")
...
pdf("F:/Plots/ImputeSingle/10x_pbmc4k/Accuracy_legend.pdf", width = 6, height = 3)
plot.new()
legend("center", 
...
load("results_10x_pbmc4k_ImputeSingle.Rdata")
pdf("F:/Plots/ImputeSingle/10x_pbmc4k/Dropout.pdf", width = 14, height = 9)
par(mfrow=c(3,4))
for(Kcluster in c(2, 5, 10)){
  for(percentage in c(0.05, 0.1, 0.2, 0.5)){
...
pdf("F:/Plots/ImputeSingle/10x_pbmc4k/Dropout_legend.pdf", width = 6, height = 4)
plot.new()
legend("center", 
