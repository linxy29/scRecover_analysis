...
for(percentage in c(0.2, 0.4, 0.6, 0.8)){
  ...
  for(Kcluster in c(2, 5, 10)){
    for(depth in c(2, 5, 10, 20, 50)){
      outputDir <- paste0("./10x_pbmc_1k_v3_ImputeSingle_p_", percentage, "_K_", Kcluster, "_d_", depth, "/")
...
for(percentage in c(0.2, 0.4, 0.6, 0.8)){
  for(Kcluster in c(2, 5, 10)){
    ...
    save(list = to_be_saved_obj, file = "./results_10x_pbmc_1k_v3_ImputeSingle.Rdata")
  }
}
