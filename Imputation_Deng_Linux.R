...
counts_original <- readRDS(file = "/data/miaozhun/analysis/imputation/Deng/Deng.rds")
...
for(percentage in c(0.01, 0.05, 0.1, 0.2)){
  ...
  for(Kcluster in c(2, 5, 10)){
    for(depth in c(2, 5, 10, 20, 50)){
      outputDir <- paste0("./Deng_ImputeSingle_p_", percentage, "_K_", Kcluster, "_d_", depth, "/")
...
for(percentage in c(0.01, 0.05, 0.1, 0.2)){
  for(Kcluster in c(2, 5, 10)){
    ...
    save(list = to_be_saved_obj, file = "./results_Deng_ImputeSingle.Rdata")
  }
}
