# Prepare data
counts <- read.csv(file = "F:/data/GSE81861/GSE81861_Cell_Line_COUNT.csv")
counts <- counts[rowSums(counts[,-1] != 0) > 0,]
genes <- unlist(lapply(strsplit(as.character(counts[,1]), "_"), "[[", 2))
counts <- counts[!duplicated(genes), ]
rownames(counts) <- genes[!duplicated(genes)]
counts <- counts[,2:ncol(counts)]
counts <- counts[rowSums(counts != 0) > 0,]

# label
label <- data.frame(cell_type1 = unlist(lapply(strsplit(colnames(counts), "__"), "[[", 2)))
rownames(label) <- colnames(counts)

saveRDS(counts, file = "F:/data/GSE81861/GSE81861_Cell_Line_COUNT.rds")
saveRDS(label, file = "F:/data/GSE81861/GSE81861_Cell_Line_label.rds")



# Plot imputation results
# PLot Accuracy
load("results_Li_ImputeSingle.Rdata")
pdf("F:/Plots/ImputeSingle/Li/Accuracy.pdf", width = 14, height = 9)
par(mfrow=c(3,4))
for(Kcluster in c(2, 5, 10)){
  for(percentage in c(0.01, 0.05, 0.1, 0.2)){
    ...
    for(depth in c(2, 5, 10, 20, 50)){
      resultsName <- paste0("Li_ImputeSingle_p_", percentage, "_K_", Kcluster, "_d_", depth)
...
pdf("F:/Plots/ImputeSingle/Li/Accuracy_legend.pdf", width = 6, height = 3)
plot.new()
legend("center", 
...
load("results_Li_ImputeSingle.Rdata")
pdf("F:/Plots/ImputeSingle/Li/Dropout.pdf", width = 14, height = 9)
par(mfrow=c(3,4))
for(Kcluster in c(2, 5, 10)){
  for(percentage in c(0.01, 0.05, 0.1, 0.2)){
...
pdf("F:/Plots/ImputeSingle/Li/Dropout_legend.pdf", width = 6, height = 4)
plot.new()
legend("center", 
