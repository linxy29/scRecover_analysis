# Prepare data
load("F:/data/GSE45719/readCounts_GSE45719.Rdata")
readCounts_GSE45719 <- readCounts_GSE45719[rowSums(readCounts_GSE45719[,-c(1,2)] != 0) > 0,]
readCounts_GSE45719 <- readCounts_GSE45719[!duplicated(readCounts_GSE45719[,1]),]
counts <- readCounts_GSE45719[,-c(1,2)]
row.names(counts) <- as.character(readCounts_GSE45719[,1])
counts <- counts[rowSums(counts != 0) > 0,]

# label
label <- unlist(lapply(strsplit(colnames(counts), "_"), "[[", 2))
label[label == "zy1" | label == "zy2" | label == "zy3" | label == "zy4"] <- "zygote"
label <- data.frame(cell_type = label)
rownames(label) <- colnames(counts)

saveRDS(counts, file = "F:/data/GSE45719/readCounts_GSE45719.rds")
saveRDS(label, file = "F:/data/GSE45719/label_GSE45719.rds")



# Plot imputation results
# PLot Accuracy
load("results_Deng_ImputeSingle.Rdata")
pdf("F:/Plots/ImputeSingle/Deng/Accuracy.pdf", width = 14, height = 9)
par(mfrow=c(3,4))
for(Kcluster in c(2, 5, 10)){
  for(percentage in c(0.01, 0.05, 0.1, 0.2)){
    ...
    for(depth in c(2, 5, 10, 20, 50)){
      resultsName <- paste0("Deng_ImputeSingle_p_", percentage, "_K_", Kcluster, "_d_", depth)
...
pdf("F:/Plots/ImputeSingle/Deng/Accuracy_legend.pdf", width = 6, height = 3)
plot.new()
legend("center", 
...
load("results_Deng_ImputeSingle.Rdata")
pdf("F:/Plots/ImputeSingle/Deng/Dropout.pdf", width = 14, height = 9)
par(mfrow=c(3,4))
for(Kcluster in c(2, 5, 10)){
  for(percentage in c(0.01, 0.05, 0.1, 0.2)){
...
pdf("F:/Plots/ImputeSingle/Deng/Dropout_legend.pdf", width = 6, height = 4)
plot.new()
legend("center", 
