# Prepare data
library(Matrix)
counts <- readMM(file = "F:/data/10x_neuron_1k_v3/filtered_feature_bc_matrix/matrix.mtx")
geneName <- read.table(file = "F:/data/10x_neuron_1k_v3/filtered_feature_bc_matrix/features.tsv", header = FALSE, stringsAsFactors = FALSE)
geneName <- geneName[,1]
cellName <- read.table(file = "F:/data/10x_neuron_1k_v3/filtered_feature_bc_matrix/barcodes.tsv", header = FALSE, stringsAsFactors = FALSE)
cellName <- cellName[,1]
row.names(counts) <- geneName
colnames(counts) <- cellName
counts <- counts[rowSums(counts != 0) > 0,]
counts <- as.matrix(counts)
saveRDS(counts, file = "F:/data/10x_neuron_1k_v3/10x_neuron_1k_v3.rds")
...
# PLot Accuracy
load("results_10x_neuron_1k_v3_ImputeSingle.Rdata")
pdf("F:/Plots/ImputeSingle/10x_neuron_1k_v3/Accuracy.pdf", width = 14, height = 9)
par(mfrow=c(3,4))
for(Kcluster in c(2, 5, 10)){
  for(percentage in c(0.2, 0.4, 0.6, 0.8)){
    ...
    for(depth in c(2, 5, 10, 20, 50)){
      resultsName <- paste0("`10x_neuron_1k_v3_ImputeSingle_p_", percentage, "_K_", Kcluster, "_d_", depth, "`")
...
pdf("F:/Plots/ImputeSingle/10x_neuron_1k_v3/Accuracy_legend.pdf", width = 6, height = 3)
plot.new()
legend("center", 
...
load("results_10x_neuron_1k_v3_ImputeSingle.Rdata")
pdf("F:/Plots/ImputeSingle/10x_neuron_1k_v3/Dropout.pdf", width = 14, height = 9)
par(mfrow=c(3,4))
for(Kcluster in c(2, 5, 10)){
  for(percentage in c(0.2, 0.4, 0.6, 0.8)){
...
pdf("F:/Plots/ImputeSingle/10x_neuron_1k_v3/Dropout_legend.pdf", width = 6, height = 4)
plot.new()
legend("center", 
