...
#for(percentage in c(0.3, 0.25, 0.35)){
#for(percentage in c(0.25, 0.35)){
  # Downsampling reads
  set.seed(999)
  ...
  counts_down <- as.matrix(counts_down)
  
  # Run ImputeSingle
  for(Kcluster in c(2, 5, 10)){
  #for(Kcluster in c(5, 10)){
...
#to_be_saved_obj <- NULL         ## original code
#to_be_saved_obj <- df_list       ## v2 code
...
save(to_be_saved_obj, file = "~/Documents/Data/scRecover/E-MTAB-3929/results_E_MTAB_3929_ImputeSingle_v2.Rdata")
